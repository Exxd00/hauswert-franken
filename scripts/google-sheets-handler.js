...
function doPost(e) {
  ...
  try {
    ...
    // Retire event queues from older browser tabs without creating another sheet.
    // Contact processing state remains on the corresponding lead row.
    if (data.recordType === 'event') {
      if (RD_EVENTS.indexOf(data.eventName) < 0) return rdJson_({ok:false,error:'invalid_event'});
      return rdJson_({ok:true,success:true,schemaVersion:2,id:id,ignored:true,storage:'analytics_only'});
    }
