...
import { getAllCitySlugs, majorCities, allCities } from '@/lib/data/cities';
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  // City + Service pages for all cities
  // This generates thousands of unique SEO pages
  const cityServicePages: MetadataRoute.Sitemap = [];

  for (const citySlug of citySlugs) {
    const isMajorCity = majorCities.some(c => c.slug === citySlug);
    for (const serviceSlug of serviceSlugs) {
      cityServicePages.push({
        url: `${BASE_URL}/${citySlug}/${serviceSlug}`,
        lastModified: new Date(),
        changeFrequency: 'monthly' as const,
        priority: isMajorCity ? 0.7 : 0.5,
      });
    }
  }
