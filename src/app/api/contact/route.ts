...
const googleSheetsUrl = process.env.GOOGLE_SHEETS_URL; // URL der Google Apps Script Web App
...
async function sendToGoogleSheets(data: ContactFormData): Promise<void> {
  if (!googleSheetsUrl) {
    console.log('Google Sheets URL nicht konfiguriert, überspringe...');
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Google Sheets Fehler:', response.statusText);
    } else {
      console.log('Daten erfolgreich an Google Sheets gesendet');
    }
  } catch (error) {
    console.error('Fehler beim Senden an Google Sheets:', error);
    // Fehler nicht werfen, damit E-Mail trotzdem gesendet wird
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Parallel: E-Mail senden UND an Google Sheets senden
    const googleSheetsPromise = sendToGoogleSheets(data);
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f4; line-height: 1.6;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color: #f5f5f4;">
    ...
    // Auf Google Sheets warten (aber Fehler ignorieren)
